...
import 'fetch_recommended_products_use_case_test.mocks.dart';

@GenerateNiceMocks([MockSpec<ProductListingRepo>()])
void main() {
  late FetchRecommendedProductsUseCase useCase;
  late MockProductListingRepo mockRepo;

  setUp(() {
    mockRepo = MockProductListingRepo();
    useCase = FetchRecommendedProductsUseCase(mockRepo);
  });
  ...
  group('fetchRecommendedProductsUseCase', () {
    setUpAll(() {
    ...
    test('should return a list of products when the repository call is successful', () async {
      ...
      when(mockRepo.fetchRecommendedProducts()).thenAnswer((_) async => Right(products));
      ...
      verify(mockRepo.fetchRecommendedProducts()).called(1);
    });
    ...
    test('should return Failure when the repository call fails', () async {
      ...
      when(mockRepo.fetchRecommendedProducts()).thenAnswer((_) async => const Left(failure));
