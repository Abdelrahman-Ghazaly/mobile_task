...
import 'fetch_most_use_products_use_case_test.mocks.dart';

@GenerateNiceMocks([MockSpec<ProductListingRepo>()])
void main() {
  late FetchMostUseProductsUseCase useCase;
  late MockProductListingRepo mockRepo;

  setUp(() {
    mockRepo = MockProductListingRepo();
    useCase = FetchMostUseProductsUseCase(mockRepo);
  });
  ...
  group('FetchMostUseProductsUseCase', () {
    const int limit = 10;
    const int offset = 0;
    ...
    test('should return a list of products when the repository call is successful', () async {
      // Arrange
      const params = FetchMostUseProductsParams(limit: limit, offset: offset);
      when(mockRepo.fetchMostUseProducts(limit: limit, offset: offset)).thenAnswer((_) async => Right(products));
      ...
      verify(mockRepo.fetchMostUseProducts(limit: limit, offset: offset)).called(1);
    });
    ...
    test('should return Failure when the repository call fails', () async {
      ...
      when(mockRepo.fetchMostUseProducts(limit: limit, offset: offset)).thenAnswer((_) async => const Left(failure));
    ...
    test('should verify that params are passed correctly', () async {
      // Arrange
      const params = FetchMostUseProductsParams(limit: 5, offset: 2);
      when(mockRepo.fetchMostUseProducts(limit: params.limit, offset: params.offset))
      ...
      // Assert
      expect(result, Right(products));
      verify(mockRepo.fetchMostUseProducts(limit: 5, offset: 2)).called(1);
    });
  });
}
